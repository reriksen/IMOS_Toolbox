...
chemistry <- read_csv(paste0(rawD,.Platform$file.sep,"chemistry.csv"), na = "(null)") %>% 
  ...
  group_by(NRScode, SampleDepth_m) %>% summarise(Silicate_umol_L = mean(Silicate_umol_L, na.rm = TRUE), # some replicated samples from error picking up PHB data, needs addressing in database
                                                 ...
                                                 Salinity = mean(Salinity, na.rm = TRUE)) %>% untibble()
...
pico <- read_csv(paste0(rawD,.Platform$file.sep,"nrs_picoplankton.csv"), na = "(null)") %>% 
  ...
  group_by(NRScode, SampleDepth_m) %>% summarise(Prochlorococcus_cells_ml = mean(Prochlorococcus_cells_ml, na.rm = TRUE), # mean of replicates
                                                 Synecochoccus_cells_ml = mean(Synecochoccus_cells_ml, na.rm = TRUE),
                                                 Picoeukaryotes_cells_ml = mean(Picoeukaryotes_cells_ml, na.rm = TRUE)) %>% untibble()
...
TSS <- read_csv(paste0(rawD,.Platform$file.sep,"nrs_TSS.csv"), na = "(null)") %>% 
  ...
  group_by(NRScode, SampleDepth_m) %>% summarise(TSS_mg_L = mean(TSS_mg_L, na.rm = TRUE), # mean of replicates
                                                 InorganicFraction_mg_L = mean(InorganicFraction_mg_L, na.rm = TRUE),
                                                 OrganicFraction_mg_L = mean(OrganicFraction_mg_L, na.rm = TRUE)) %>% untibble()
...
comb <- trips %>% left_join(biomass %>% select(NRScode, SampleDepth_m, Biomass_mgm3), by = c("NRScode", "SampleDepth_m")) %>%
...
test <- comb %>% group_by(NRScode, SampleDepth_m) %>% summarise(n = n())
